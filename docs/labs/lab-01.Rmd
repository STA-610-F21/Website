---
title: "Lab 1: Help, these kids are killing me! (Part I)"
...
# Introduction
This individual lab is a quick assessment to gauge your level of comfort with doing exploratory data analysis (in R!) and specifying linear regression models for small data. Since we are yet to cover any rigorous material, your goal is to specify a basic linear regression model to the data below and interpret your results. This should be a relatively short lab, so you should also use it as an opportunity to figure out the logistics of future labs, that is, check your R/RStudio installation, make sure you know how to work in R markdown and knit to .pdf, and so on.
...
1. Create two plots:
    ...
    Report your findings from the plots. What can you conclude about the relationships?
2. Now create a single plot to visualize the lifespan (longevity) of the male fruit flies as a function of sexual activity but broken down by thorax length. What can you conclude about this plot?
3. Specify a two-way ANOVA model without interactions for estimating the association between longevity (your response variable), sexual activity, and thorax length in male fruit flies.
4. Fit the model in R and interpret your findings. Be sure to use language accessible to a general scientifically-savvy reader (e.g., someone reading a science article in the BBC or CNN apps).
5. 
