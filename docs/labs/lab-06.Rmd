...
4. Identify three instructor-level predictors excluding `beauty` (with `profnumber` being the instructor-level identifier)  that you think we should control for based on EDA. Write down and fit a varying/random intercepts model for the data grouped by `profnumber`, and include `beauty`, plus the other three variables you have identified as potential predictors. 
...
6. For the model from question 4, try out a few other choices for the default hyper-priors. That is, fit the model again using your own choices for the parameters of the prior distributions and report your findings. Did any of the estimates change? If yes, how?

7. We will cover how to incorporate additional grouping variables in more detail soon but for now, extend the model from question 4 to allow for varying/random intercepts by `CourseID` and varying/random slopes for `beauty` by `CourseID`. To implement this in `R`, just add "`+ (beauty | CourseID)`" to your existing formula.

8. Fit the model using the default prior options in `brms`. Did any of the results for the fixed effects change? If yes, why do you think that is?
